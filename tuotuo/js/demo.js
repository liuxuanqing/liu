...
$(function(){
    ...
    //     "autoplay":1000,//自动切换的时间间隔（单位ms）
    //     ///"loop":true,//循环切换 loop模式下会在slides前后复制若干个slide。默认为0，前后各复制1个。0,1,2 --> 2,0,1,2,0
    //     "effect":"fade",//切换效果
    //     "pagination":".swiper-pagination",//分页器
    //     "paginationBulletRender": function (swiper, index, className) {//显示分页器索引值
    //         return '<span class="' + className + '">' + (index + 1) + '</span>';
    //     },
    //     //"paginationClickable" :true,//点击分页器，显示对应slide
    // });
    //鼠标悬浮分页器上显示对应的slide
    //
    // (function showSlide(){
    //     $.each($(".swiper-pagination span"),function(i,n){//i是索引值 n是每个索引值对应的值
    //         $(n).mouseenter(function(){
    //             //console.log(i);
    //             mySwiper.slideTo(i+1);//因为循环状态下默认前后各增加一个，所以要加1
    //         });
    //     });
    // })();
    ...
    (function(){
        ...
        function effect(index){
            ...
            */
            //用$.each()方法
            $.each($(".player .slide"),function(i,value){//注意这里得到的是每个dom元素
                $(value).css("display","none").css("opacity",0.2);
                //console.log($(value) instanceof jQuery);//true检测一个元素是不是jQuery对象
            });
            //显示当前元素
